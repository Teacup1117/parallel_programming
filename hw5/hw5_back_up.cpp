...
// 優化參數
#define SMALL_N_THRESHOLD 256   // 只有非常小的 N 使用純 shared memory
#define BATCH_STEPS 10000       // 極大化 batch size
#define BATCH_STEPS_LARGE 200   // Large N 的批處理步數（受限於計算時間）
...
// =========================================================
// Kernel 3: Large N Batched (Tiled + Multiple Steps per Kernel)
// 每個 warp group 處理一個 scenario，使用 tiled 方法但在 kernel 內跑多步
// =========================================================
__global__ void solve_batch_large_n(
    int n, int start_step, int steps_to_run,
    int planet, int asteroid,
    const int* __restrict__ target_device_ids,
    const int* __restrict__ type,
    SimStatus* global_results,
    double* __restrict__ state_in,
    double* __restrict__ state_out,
    int global_scenario_offset
) {
    __shared__ double sh_qx[BLOCK_SIZE];
    ...
    __shared__ double sh_qz[BLOCK_SIZE];
    __shared__ double sh_m[BLOCK_SIZE];
    __shared__ double sh_planet_pos[3];
    __shared__ double sh_asteroid_pos[3];
    __shared__ int sh_destroyed;
    __shared__ int sh_destroy_step;
    __shared__ int sh_hit_step;
    __shared__ double sh_min_dist;

    int tid = threadIdx.x;
    int scenario_idx = blockIdx.x;  // one block per scenario
    int global_scenario_idx = scenario_idx + global_scenario_offset;
    int target_dev_id = target_device_ids[scenario_idx];
    size_t state_offset = (size_t)scenario_idx * 7 * n;
    ...
    // 載入初始狀態
    if (tid == 0) {
        sh_destroyed = global_results[scenario_idx].device_destroyed;
        sh_destroy_step = global_results[scenario_idx].destroy_step;
        sh_hit_step = global_results[scenario_idx].hit_time_step;
        sh_min_dist = global_results[scenario_idx].min_dist;
    }
    __syncthreads();

    double* ptr_in = state_in;
    double* ptr_out = state_out;
    ...
    for (int s = 0; s < steps_to_run; ++s) {
        ...
        // 處理所有粒子 (一個 block 處理所有粒子，但每次只處理 BLOCK_SIZE 個)
        for (int my_tile = 0; my_tile < num_tiles; my_tile++) {
            int i = my_tile * BLOCK_SIZE + tid;
            
            double my_qx = 0, my_qy = 0, my_qz = 0;
            double my_vx = 0, my_vy = 0, my_vz = 0;
            
            if (i < n) {
                my_qx = ptr_in[state_offset + 0*n + i];
                my_qy = ptr_in[state_offset + 1*n + i];
                my_qz = ptr_in[state_offset + 2*n + i];
                my_vx = ptr_in[state_offset + 3*n + i];
                my_vy = ptr_in[state_offset + 4*n + i];
                my_vz = ptr_in[state_offset + 5*n + i];
            }
            ...
            for (int tile = 0; tile < num_tiles; tile++) {
                ...
                if (tile_idx < n) {
                    sh_qx[tid] = ptr_in[state_offset + 0*n + tile_idx];
                    sh_qy[tid] = ptr_in[state_offset + 1*n + tile_idx];
                    sh_qz[tid] = ptr_in[state_offset + 2*n + tile_idx];
                    double raw_m = ptr_in[state_offset + 6*n + tile_idx];
                    if (raw_m > 0.0 && type[tile_idx] == param::TYPE_DEVICE)
                        sh_m[tid] = get_device_mass(raw_m, current_time);
                    else
                        sh_m[tid] = raw_m;
            ...
            // 更新速度和位置
            if (i < n) {
                my_vx += acc_x * param::dt;
                my_vy += acc_y * param::dt;
                my_vz += acc_z * param::dt;
                my_qx += my_vx * param::dt;
                my_qy += my_vy * param::dt;
                my_qz += my_vz * param::dt;
                
                ptr_out[state_offset + 0*n + i] = my_qx;
                ptr_out[state_offset + 1*n + i] = my_qy;
                ptr_out[state_offset + 2*n + i] = my_qz;
                ptr_out[state_offset + 3*n + i] = my_vx;
                ptr_out[state_offset + 4*n + i] = my_vy;
                ptr_out[state_offset + 5*n + i] = my_vz;
                ptr_out[state_offset + 6*n + i] = ptr_in[state_offset + 6*n + i];
            }
            __syncthreads();
        }
        
        // 載入 planet 和 asteroid 位置做事件檢查
        if (tid == 0) {
            sh_planet_pos[0] = ptr_out[state_offset + 0*n + planet];
            sh_planet_pos[1] = ptr_out[state_offset + 1*n + planet];
            sh_planet_pos[2] = ptr_out[state_offset + 2*n + planet];
            sh_asteroid_pos[0] = ptr_out[state_offset + 0*n + asteroid];
            sh_asteroid_pos[1] = ptr_out[state_offset + 1*n + asteroid];
            sh_asteroid_pos[2] = ptr_out[state_offset + 2*n + asteroid];
        }
        __syncthreads();
        
        // 事件檢查 (只用一個 thread)
        if (tid == 0) {
            double dx = sh_planet_pos[0] - sh_asteroid_pos[0];
            double dy = sh_planet_pos[1] - sh_asteroid_pos[1];
            double dz = sh_planet_pos[2] - sh_asteroid_pos[2];
            double dist_sq = dx*dx + dy*dy + dz*dz;
            
            if (global_scenario_idx == 0) {
                double dist = sqrt(dist_sq);
                if (dist < sh_min_dist) sh_min_dist = dist;
            }
            
            if (sh_hit_step == -2 && dist_sq < param::planet_radius_sq) {
                sh_hit_step = current_step;
            }
            
            if (target_dev_id >= 0 && sh_destroyed == 0) {
                double m_dist = current_step * param::dt * param::missile_speed;
                double dev_x = ptr_out[state_offset + 0*n + target_dev_id];
                double dev_y = ptr_out[state_offset + 1*n + target_dev_id];
                double dev_z = ptr_out[state_offset + 2*n + target_dev_id];
                
                double d_dx = dev_x - sh_planet_pos[0];
                double d_dy = dev_y - sh_planet_pos[1];
                double d_dz = dev_z - sh_planet_pos[2];
                double d_dist = sqrt(d_dx*d_dx + d_dy*d_dy + d_dz*d_dz);
                
                if (m_dist > d_dist) {
                    sh_destroyed = 1;
                    sh_destroy_step = current_step;
                    ptr_out[state_offset + 6*n + target_dev_id] = 0.0;
                }
            }
        }
        __syncthreads();
    ...
    // 寫回結果
    if (tid == 0) {
        global_results[scenario_idx].min_dist = sh_min_dist;
        global_results[scenario_idx].hit_time_step = sh_hit_step;
        global_results[scenario_idx].device_destroyed = sh_destroyed;
        global_results[scenario_idx].destroy_step = sh_destroy_step;
    }
    
    // 如果跑了奇數步，需要複製回 state_in
    if ((steps_to_run % 2) == 1) {
        for (int my_tile = 0; my_tile < num_tiles; my_tile++) {
            int i = my_tile * BLOCK_SIZE + tid;
            if (i < n) {
                state_in[state_offset + 0*n + i] = ptr_in[state_offset + 0*n + i];
                state_in[state_offset + 1*n + i] = ptr_in[state_offset + 1*n + i];
                state_in[state_offset + 2*n + i] = ptr_in[state_offset + 2*n + i];
                state_in[state_offset + 3*n + i] = ptr_in[state_offset + 3*n + i];
                state_in[state_offset + 4*n + i] = ptr_in[state_offset + 4*n + i];
                state_in[state_offset + 5*n + i] = ptr_in[state_offset + 5*n + i];
                state_in[state_offset + 6*n + i] = ptr_in[state_offset + 6*n + i];
            }
            __syncthreads();
        }
    }
...
void gpu_worker_large_n(GPUResources& res, int n, int planet, int asteroid) {
    ...
    hipLaunchKernelGGL(initialize_scenarios_kernel, init_grid, BLOCK_SIZE, 0, res.stream,
        n, res.num_scenarios, res.d_qx_bk, res.d_qy_bk, res.d_qz_bk,
        res.d_vx_bk, res.d_vy_bk, res.d_vz_bk, res.d_m_bk, res.d_type,
        res.d_state_1, res.scenario_start);
    
    // Large N 使用原本的 per-step kernel (正確的並行度)
    int blocks_physics = (n + BLOCK_SIZE - 1) / BLOCK_SIZE;
    dim3 physics_grid(blocks_physics, res.num_scenarios);
    
    double* d_in = res.d_state_1;
    double* d_out = res.d_state_2;
    
    for (int step = 1; step <= param::n_steps; step++) {
        double current_time = step * param::dt;
        
        hipLaunchKernelGGL(update_physics_large_n_parallel, physics_grid, BLOCK_SIZE, 0, res.stream,
            n, current_time, res.d_type, d_in, d_out);
        
        hipLaunchKernelGGL(check_events_large_n_parallel, res.num_scenarios, 1, 0, res.stream,
            step, n, planet, asteroid, res.d_dev_map, d_out, res.d_results, res.scenario_start);

...
void solve(FILE *fin, FILE *fout)
{
    ...
    int blocksPerGrid = 2048;  // Increased for better GPU utilization
    
    // Search in batches - increased batch size for better performance
    unsigned int batch_size = 10000000;  // 50M nonces per batch (optimized)
    ...
    while(start_nonce <= 0xffffffff && !found_flag)
    {
        ...
        if(found_flag)
        {
            ...
            printf("Found in batch #%d (searched %.0f million nonces)\n", 
                   batch_count, (double)start_nonce / 1000000.0);
            break;
        }
    ...
    printf("Total search time: %ld ms\n\n", search_duration.count());
    ...
    printf("Total solve time: %ld ms\n\n", solve_duration.count());
}

int main(int argc, char **argv)
{
    // Performance measurement - overall program

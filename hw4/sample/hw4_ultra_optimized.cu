...
// Optimized CUDA Kernel for parallel nonce search
...
)
{
    ...
    for(unsigned int offset = tid; offset < total_nonces; offset += stride)
    {
        ...
        // Create local copy of block (minimize memory access)
...
void solve(FILE *fin, FILE *fout)
{
    ...
    // GPU computation with optimized configuration
    HashBlock *d_block;
    ...
    // Optimized launch configuration - maximize occupancy
    int minGridSize, threadsPerBlock;
    cudaOccupancyMaxPotentialBlockSize(&minGridSize, &threadsPerBlock, 
                                       find_nonce_kernel, 0, 0);
    
    // Use multiple SMs efficiently - increase grid size for better parallelism
    int blocksPerGrid = minGridSize * 4;  // 4x oversubscription for better latency hiding
    
    // Larger batch size to reduce kernel launch overhead
    unsigned int batch_size = 100000000;  // 100M nonces per batch
    unsigned int start_nonce = 0;
    
    // printf("Searching for nonce using GPU (threads=%d, blocks=%d)...\n", 
    ...
    while(start_nonce <= 0xffffffff && !found_flag)
    {
        ...
        // Check if solution found (cudaMemcpy automatically synchronizes)

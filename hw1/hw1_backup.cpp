...
int calculate_heuristic_bitstate_hungarian(const BitState& bit_state) {
    ...
    if (boxes_idx.size() != free_targets.size()) {
        return calculate_heuristic_bitstate_enhanced(bit_state); // fallback safety
    }
    int n = (int)boxes_idx.size();
    vector<vector<int>> cost(n, vector<int>(n));
    for (int i = 0; i < n; ++i) {
        const auto &cell = g_movable_cells[boxes_idx[i]]; int flat = cell.first * g_width + cell.second;
        for (int j = 0; j < n; ++j) cost[i][j] = g_target_dists[free_targets[j]][flat];
    }
...
int calculate_heuristic_bitstate_enhanced(const BitState& bit_state) {
    ...
    collect_boxes_and_free_targets(bit_state, boxes_idx, free_targets);
    if (boxes_idx.empty()) return 0;
    // Greedy: for each box pick nearest free target
    int total = 0; vector<char> used(free_targets.size(), 0);
    for (int bi = 0; bi < (int)boxes_idx.size(); ++bi) {
        const auto &cell = g_movable_cells[boxes_idx[bi]]; int flat = cell.first * g_width + cell.second;
        int best = INT_MAX, bestj = -1;
        for (int j = 0; j < (int)free_targets.size(); ++j) if (!used[j]) {
            int dist = g_target_dists[free_targets[j]][flat];
            if (dist < best) { best = dist; bestj = j; }
        }
        if (bestj >= 0) { used[bestj] = 1; total += best; }
    }
    return total;
}
...
int calculate_heuristic_bitstate_adaptive(const BitState& bit_state) {
    if (bit_state.boxes_on_floor.none()) return 0;
    int num_unsolved = (int)bit_state.boxes_on_floor.count();
    if (num_unsolved <= 8) return calculate_heuristic_bitstate_hungarian(bit_state);
    if (num_unsolved <= 15) return calculate_heuristic_bitstate_enhanced(bit_state);
    vector<int> boxes_idx, free_targets; boxes_idx.reserve(64); free_targets.reserve(64);
    collect_boxes_and_free_targets(bit_state, boxes_idx, free_targets);
    sort(boxes_idx.begin(), boxes_idx.end(), [](int a, int b){
        const auto &A = g_movable_cells[a]; const auto &B = g_movable_cells[b];
        return (A.first==B.first)? A.second<B.second : A.first<B.first;
    });
    int total = 0; vector<char> used(free_targets.size(), 0);
    for (int bi : boxes_idx) {
        const auto &cell = g_movable_cells[bi]; int flat = cell.first * g_width + cell.second;
        int best = INT_MAX, bestj = -1, checked=0;
        for (int j = 0; j < (int)free_targets.size(); ++j) if (!used[j]) {
            int dist = g_target_dists[free_targets[j]][flat];
            if (dist < best) { best = dist; bestj = j; }
            if (++checked >= 8) { /* limit */ }
...
string astar_solver(const string& filename) {
    ...
    AStarNode start_node; start_node.bit_state = compress_to_bitstate(initial_state);
    start_node.path = ""; start_node.g_cost = 0;
    start_node.h_cost = calculate_heuristic_bitstate_adaptive(start_node.bit_state);
    start_node.last_move = 0; start_node.last_was_push = false;
    open_set.push(start_node);
    visited_states.insert(start_node.bit_state);
    int nodes_expanded = 0;
    while (!open_set.empty()) {
        AStarNode current = open_set.top(); open_set.pop();
        ++nodes_expanded;
        if (is_solved(current.bit_state)) {
            cerr << "A* solved in " << nodes_expanded << " nodes" << endl;
            return current.path;
        }
        if (is_deadlock(current.bit_state)) continue;
        auto successors = generate_macro_moves(current.bit_state);
        vector<pair<int,int>> scored; scored.reserve(successors.size());
        ...
        for (auto &pr : scored) {
            int idx = pr.second; const auto &pairRef = successors[idx];
            const BitState &nb = pairRef.first; const string &mp = pairRef.second;
            if (!visited_states.insert(nb).second) continue;
            AStarNode nn; nn.bit_state=nb; nn.path=current.path+mp; nn.g_cost=current.g_cost+1; nn.h_cost=pr.first;
            nn.last_move = mp.back(); nn.last_was_push = true; open_set.push(std::move(nn));
        }
    }
...
string astar_solver_parallel(const string& filename, int num_threads) {
    ...
    PQ open_set; unordered_set<BitState, BitStateHash> visited;
    AStarNode start; start.bit_state = compress_to_bitstate(initial_state);
    start.path=""; start.g_cost=0; start.h_cost=calculate_heuristic_bitstate_adaptive(start.bit_state);
    open_set.push(start); visited.insert(start.bit_state);
    mutex pq_mutex, visited_mutex; condition_variable pq_cv; atomic<bool> solved(false);
    string solution_path; atomic<long long> nodes_expanded{0};
    auto worker = [&](){
        constexpr int CHUNK_SIZE = 32; vector<AStarNode> batch; batch.reserve(CHUNK_SIZE);
        vector<AStarNode> new_nodes; new_nodes.reserve(256);
        while (!solved.load(memory_order_relaxed)) {
            batch.clear();
            {
                unique_lock<mutex> lk(pq_mutex);
                pq_cv.wait(lk,[&]{return solved.load(memory_order_relaxed)||!open_set.empty();});
                if (solved.load(memory_order_relaxed)) return;
                for (int i=0;i<CHUNK_SIZE && !open_set.empty();++i){ batch.push_back(open_set.top()); open_set.pop(); }
                if (batch.empty()) continue;
            }
            for (auto &current : batch) {
                if (solved.load(memory_order_relaxed)) break;
                nodes_expanded.fetch_add(1, memory_order_relaxed);
                if (is_solved(current.bit_state)) { solution_path=current.path; solved.store(true); pq_cv.notify_all(); return; }
                if (is_deadlock(current.bit_state)) continue;
                auto successors = generate_macro_moves(current.bit_state);
                for (auto &pr : successors) {
                    if (solved.load(memory_order_relaxed)) break;
                    const BitState &nb = pr.first; const string &mp = pr.second;
                    if (is_deadlock(nb)) continue;
                    bool inserted=false; {
                        lock_guard<mutex> g(visited_mutex);
                        if (visited.find(nb)==visited.end()) { visited.insert(nb); inserted=true; }
                    }
                    if (!inserted) continue;
                    int h = calculate_heuristic_bitstate_adaptive(nb);
                    AStarNode nn; nn.bit_state=nb; nn.path=current.path+mp; nn.g_cost=current.g_cost+1; nn.h_cost=h; nn.last_move=mp.back(); nn.last_was_push=true;
            ...
            if (!new_nodes.empty()) {
                {
                    ...
                    for (auto &n : new_nodes) open_set.push(std::move(n));
                }
                new_nodes.clear();
                pq_cv.notify_all();
            }
    ...
    if (!solved.load()) throw Oops("Parallel A* failed to find solution");
    cerr << "Parallel A* solved in " << nodes_expanded.load() << " node expansions using " << num_threads << " threads" << endl;

...
struct WorkItem {
    ...
    bool operator<(const WorkItem& other) const {
        return priority > other.priority; // 較小的優先級值有較高優先權
    }
...
private:
    // 使用優先佇列而不是deque來提高插入效率
    priority_queue<WorkItem, vector<WorkItem>, greater<WorkItem>> queue;
    mutable mutex mtx;
...
public:
    // 批次推送多個工作項目
    void push_batch(const vector<WorkItem>& items) {
        if (finished.load() || items.empty()) return;
    ...
    // 批次彈出多個工作項目
    size_t pop_batch(vector<WorkItem>& items, size_t max_items = BATCH_SIZE, int timeout_ms = 50) {
        items.clear();
        unique_lock<mutex> lock(mtx);
        
        if (cv.wait_for(lock, chrono::milliseconds(timeout_ms), 
                       [this] { return !queue.empty() || finished.load(); })) {
            
            size_t count = 0;
            while (!queue.empty() && count < max_items) {
    ...
    bool pop(WorkItem& item, int timeout_ms = 50) {
        unique_lock<mutex> lock(mtx);
        if (cv.wait_for(lock, chrono::milliseconds(timeout_ms), 
                       [this] { return !queue.empty() || finished.load(); })) {
            if (!queue.empty()) {
...
struct ThreadLocalData {
    ...
    // 本地工作佇列來減少全局佇列競爭
    priority_queue<WorkItem, vector<WorkItem>, greater<WorkItem>> local_queue;
    size_t work_steal_threshold = 50; // 當本地佇列超過此大小時，向全局佇列貢獻工作
    
    ThreadLocalData(int id) : thread_id(id) {}
    ...
    // 從本地佇列獲取工作
    bool get_local_work(WorkItem& item) {
        if (!local_queue.empty()) {
            item = local_queue.top();
            local_queue.pop();
            return true;
        }
        return false;
    ...
    // 添加工作到本地佇列
    void add_local_work(const WorkItem& item) {
        local_queue.push(item);
    }
    
    // 將部分本地工作轉移到全局佇列（work stealing預防）
    void balance_work(); // 實現將在後面定義
};

static ThreadSafeWorkQueue global_work_queue;

// ThreadLocalData::balance_work的實現
void ThreadLocalData::balance_work() {
    if (local_queue.size() > work_steal_threshold) {
        vector<WorkItem> items_to_share;
        size_t share_count = local_queue.size() / 2;
        
        while (items_to_share.size() < share_count && !local_queue.empty()) {
            items_to_share.push_back(local_queue.top());
            local_queue.pop();
        }
        
        if (!items_to_share.empty()) {
            global_work_queue.push_batch(items_to_share);
        }
    }
}
...
CompressedBoxState compress_boxes(const GameState& state) {
    ...
    // Initialize with base map characters (empty floor ' ' or empty target '.')
    for (int i = 0; i < (int)g_movable_cells.size(); ++i) {
        int y = g_movable_cells[i].first;
        int x = g_movable_cells[i].second;
        compressed.boxes[i] = g_base_map[y][x];
    }
    
    // Override with box positions
...
void worker_thread(ThreadLocalData& local_data) {
    active_threads++;
    
    vector<WorkItem> batch_buffer; // 批次緩衝區
    ...
    while (!solution_found.load()) {
        ...
        // 1. 首先嘗試從本地佇列獲取工作
        if (local_data.has_local_work()) {
            got_work = local_data.get_local_work(work_item);
        }
        
        // 2. 如果本地沒有工作，嘗試從全局佇列批次獲取工作
        if (!got_work) {
            batch_buffer.clear();
            size_t batch_count = global_work_queue.pop_batch(batch_buffer, 8, 25);
            
            if (batch_count > 0) {
                // 取第一個工作項目，其餘加入本地佇列
                work_item = batch_buffer[0];
                got_work = true;
                
                for (size_t i = 1; i < batch_count; ++i) {
                    local_data.add_local_work(batch_buffer[i]);
                }
            } else {
                // 仍然沒有工作，檢查是否應該退出
                if (global_work_queue.empty() && active_threads.load() == 1) {
                    break;
                }
                continue;
            }
        ...
        AStarNode current = work_item.node;
        local_data.nodes_expanded++;
        ...
        if (is_solved(current.state)) {
            ...
            if (!solution_found.load()) {
                ...
                global_work_queue.finish();
                cerr << "Thread " << local_data.thread_id << " found solution after " 
                     << local_data.nodes_expanded << " nodes" << endl;
            }
            break;
        }
        ...
        // Generate successors
        vector<WorkItem> new_work_items;
        new_work_items.reserve(4); // 最多4個方向
        ...
        for (const auto& [key, direction] : DYDX) {
            ...
            // Determine if this move would be a push
            ...
            // Check if this box configuration with this player position has been visited by this thread
            bool skip_node = false;
            auto box_it = local_data.visited_box_states.find(new_compressed_boxes);
            if (box_it != local_data.visited_box_states.end()) {
            ...
            local_data.visited_box_states[new_compressed_boxes].insert(new_player_index);
            ...
            new_work_item.node = new_node;
            new_work_item.priority = new_node.f_cost();
            new_work_items.push_back(new_work_item);
        }
        
        // 將新工作項目加入本地佇列
        for (const auto& item : new_work_items) {
            if (solution_found.load()) break;
            local_data.add_local_work(item);
        }
        
        // 定期進行工作平衡
        if (local_data.nodes_expanded % 100 == 0) {
            local_data.balance_work();
        }
    }
    
    // 線程結束時，將剩餘的本地工作轉移到全局佇列
    vector<WorkItem> remaining_work;
    WorkItem item;
    while (local_data.get_local_work(item)) {
        remaining_work.push_back(item);
    }
    if (!remaining_work.empty()) {
        global_work_queue.push_batch(remaining_work);
    }
    
    active_threads--;
}

// Multi-threaded A* solver
string parallel_astar_solver(const string& filename, int num_threads = 6) {
    ...
    initial_work.node = start_node;
    initial_work.priority = start_node.f_cost();
    global_work_queue.push(initial_work);
    ...
    for (int i = 0; i < num_threads; ++i) {
        thread_data.emplace_back(make_unique<ThreadLocalData>(i));
        threads.emplace_back([&, i]() {
            worker_thread(*thread_data[i]);
        });
    }
    ...
    for (const auto& data : thread_data) {
        total_nodes += data->nodes_expanded;
    }
    cerr << "Parallel A* expanded " << total_nodes << " total nodes across " 
    ...
    while (!open_set.empty()) {
        ...
        for (const auto& [key, direction] : DYDX) {
            ...
            if (box_it != visited_box_states.end()) {
                // Always check if exact same box+player configuration was visited
                if (box_it->second.find(new_player_index) != box_it->second.end()) {
                    skip_node = true;
                }
            }
...
// Main solver using parallel A*
string main_solver(const string& filename) {
    return parallel_astar_solver(filename, 6);
}
